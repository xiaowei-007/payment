 ...
 * @date    : 2020/4/08 17:13
 * @version : 1.0.0
 * @desc    : 当面付付款码解码获取用户id
 **/
class PayCodeDecode extends AliBaseObject implements IGatewayRequest
{
    const METHOD = 'alipay.marketing.facetoface.decode.use';
    ...
    {
        $bizContent = [
            'dynamic_id' => $requestParams['auth_code'] ?? '',
            'sence_no'     => $requestParams['sence_no'] ?? '',
        ];
    ...
    public function request(array $requestParams)
    {
        try {
            ...
            $content = $retArr['alipay_trade_payCodeDecode_response'];
